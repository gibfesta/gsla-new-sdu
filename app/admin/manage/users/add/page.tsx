...
| - "Create User" only changes local UI status (no backend)
|
| Where to wire backend later:
| - Replace `createUserUiOnly()` with server action / API call:
|   1) Create Identity/User
|   2) Create Profile fields
|   3) Optional: create membership(s) + team/role assignments
|   4) Optional: send invite / welcome email
|--------------------------------------------------------------------------
*/
...
const DRAFT_KEY = `gsla_admin_manage_create_user_draft_v1`;
...
type State = {
  ...
  membership: {
    ...
    teamRoles: string; // free text for now
  };
...
/* Small local “label” helper so we don’t depend on shadcn Label/Input modules */
...
/* Simple checkbox row used throughout the wizard */
function CheckRow({
  checked,
  onChange,
  title,
  ...
  return (
    <label className={cn("flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-3", disabled && "opacity-60")}>
      <input
        type="checkbox"
        checked={checked}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px_1fr]">
        ...
        <Card>
          <CardContent className="p-6">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {(
                      ...
                      <label key={r} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2">
                        <input type="checkbox" checked={state.roles.includes(r)} onChange={() => toggleRole(r)} className="h-4 w-4" />
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-3">
                      <CheckRow
                        ...
                        onChange={(v) =>
                          update("volunteer", { ...state.volunteer, compliance: { ...state.volunteer.compliance, dbsValid: v } })
                        }
                      ...
                      <CheckRow
                        ...
                        onChange={(v) =>
                          update("volunteer", { ...state.volunteer, compliance: { ...state.volunteer.compliance, safeguardingValid: v } })
                        }
                      ...
                      <CheckRow
                        ...
                        onChange={(v) =>
                          update("volunteer", { ...state.volunteer, compliance: { ...state.volunteer.compliance, firstAidValid: v } })
                        }
            ...
            {/* DOCUMENTS + INTERNAL NOTE */}
            {step.key === "documents" && (
              <div className="space-y-6">
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="text-sm font-semibold text-slate-800">Document placeholders</div>
                  <div className="mt-2 text-xs text-slate-600">UI-only. Later replace with real uploads + expiry tracking.</div>
            ...
            {step.key === "review" && (
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4">
                  <div>
                    <div className="text-sm font-semibold text-slate-800">Create</div>
                    <div className="mt-1 text-xs text-slate-600">UI-only. Backend later creates user + memberships.</div>
                    {missing.length ? (
                ...
                {created ? (
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                    ...
                    <div className="mt-1 text-xs text-emerald-900/80">
                      Next: wire backend → redirect to /admin/users/[id] and add memberships/requests tables.
                    </div>
                  </div>
                ) : null}
              </div>
